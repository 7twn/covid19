...
#
# 【啟動方式】在本檔所在資料夾的終端機輸入：
#   D:\thonny5\python.exe -m streamlit run app_c19n.py
# 請把 covid-19all.csv 放在本檔旁邊；不是直接按一般 Python 程式的執行鍵。
# 套件：streamlit、pandas、numpy、plotly；請沿用目前已測試的 Thonny 5 Python 環境。
...
# 裝飾器快取相同參數的結果，選單改變時不用一直重讀 CSV。
# modified_time 在函式呼叫時參與快取識別，因此函式內 del 它也不影響更新判斷。
@st.cache_data(show_spinner="正在載入確診資料…")
def load_data(path: str, modified_time: int) -> pd.DataFrame:
    """載入必要欄位；modified_time 讓檔案更新後自動刷新快取。"""
    del modified_time
    # usecols 只讀所需欄位，節省記憶體；DataFrame 保留原始每筆彙整紀錄。
    columns = [YEAR_COL, WEEK_COL, CITY_COL, SEX_COL, IMPORT_COL, AGE_COL, CASE_COL]
    data = pd.read_csv(path, usecols=columns, encoding="utf-8", low_memory=False)
...
# 防衛式處理：缺檔就顯示錯誤並 st.stop()，避免後續產生連鎖例外。
# 保留原版提示中的 app_c19.py 名稱；實際 CSV_PATH 依本檔所在資料夾計算。
if not CSV_PATH.exists():
    st.error(f"找不到資料檔：{CSV_PATH.name}。請將 CSV 放在 app_c19.py 同一資料夾。")
    st.stop()

# st_mtime_ns 是檔案最後修改時間（奈秒），讓 CSV 更新後重新載入。
# except 捕捉列出的讀檔／格式問題，其餘程式錯誤仍會顯示，方便除錯。
try:
    df = load_data(str(CSV_PATH), CSV_PATH.stat().st_mtime_ns)
except (UnicodeDecodeError, ValueError, OSError) as exc:
    st.error(f"CSV 載入失敗：{exc}")
    st.stop()
...
def render_sidebar_filters():
    ...
    st.divider()
    st.caption(f"資料來源：{CSV_PATH.name}")
